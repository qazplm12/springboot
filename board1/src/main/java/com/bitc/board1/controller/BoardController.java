...
// @Controller : 일반적인 SOAP 방식의 HTTP 응답 처리를 할 경우 사용함, get/post 2가지 사용
...
public class BoardController {
    ...
    // get/post 와 같은 주소 및 다른 추가 옵션을 url과 함께 사용 시 옵션명을 반드시 입력
    ...
    // 클래스에 @RequestMapping을 사용했을 경우 해당 클래스의 전체 주소를 입력하고,
    // 멤버 메소드에는 상세주소를 입력하는 형식으로 사용할 수 있음
    ...
    // 반환타입이 ModelAndView 일 경우는 templates의 html 파일에 데이터가 포함되어 있음을 의미함
    // 게시판 전체 리스트 출력
    @RequestMapping(value = "/board/boardList.do", method = RequestMethod.GET)
    public ModelAndView selectBoardList() throws Exception {
        // ModelAndView : 데이터와 View Templates이 모두 모두 포함된 클래스
        ...
        mv.addObject("apple", "아이폰 15");
        mv.addObject("samsung", "갤럭시 S23");

        return mv;
    }

    // @RequestParam : 클라이언트에서 전달된 파라미터값을 지정하는 어노테이션
    // 파라미터 하나에 데이터 1개가 전달되는 형식일때 사용
    // 전달되는 파라미터의 이름과 메소드 매개변수의 이름을 다르게 사용하고자 할 경우
    // @RequestParam 어노테이션 뒤에 (value= '파라미터명')을 사용함

    // 지정한 게시글 상세 보기
    @RequestMapping("/board/openBoardDetail.do")
    public ModelAndView selectBoardDetails(@RequestParam int boardIdx) throws Exception {
        ...
        // 가져온 데이터 정보를 ModelAndView 클래스 타입의 객체에 저장
    ...
    // 게시판 글 쓰기(db에 insert)
    // 클라이언트로부터 전달된 파일 정보를 읽어오기 위해서 MultipartHttpServletRequest 타입의 매개변수 추가
