...
public interface ProductRepository extends JpaRepository<ProductEntity, Long> {
    ...
    // 쿼리 메소드 : JpaRepository에서 제공하는 기본 메소드로는 원하는 데이터를 모두 컨트롤하기 힘들기 때문에
    // JpaRepository에서 제공하는  주제 키워드를 조합하여 원하는 메소드를 생성하여 사용할 수 있음
    ...
    boolean existsByNumber(Long number); // 지정된 번호의 데이터가 있는지 조회
    ...
    List<ProductEntity> findTop5ByNumber(Long number); // 조회된 결과 중 숫자가 큰 것 5개만 출력
    ...
    // (Is)Not : 값의불일치를 조건으로 사용하는 조건자 키워드, IS를 생략할 수 있음
    ProductEntity findByNumberIsNot(Long number); // 지정한 번호와

    ProductEntity findByNumberNot(Long number); // 지정한 번호와 같지 않은 테이블 조회

    // (Is)Null, (Is)NotNull : 값이 null인지 확인하는 조건자 키워드, IS 생략 가능
//    List<ProductEntity> findByUpdatedAtNull();  // 수정된 데이터가 Null 인 값만 조회
...
//    List<ProductEntity> findByIsActiveIsFalse();
    ...
    // And, Or : 조건을 추가하는 키워드
    ProductEntity findByNumberAndName(Long number, String name); // and 명령으로 조회 조건을  추가,

    // 지정한 번호와 지정한 이름이 같은 데이터를 조회
    ProductEntity findByNumberOrName(Long number, String name); // or 명령으로 조회 조건을  추가,
    // 지정한 번호나 지정한 이름이 같은 데이터를 조회

    // greaterThan, (Is)LessThan, (Is)Between : 숫자나 datetime 컬럼을 대상으로 비교 연산에 사용할 수 있는 조건자 키워드
    // greater Than, LessThan은 비교 대상에 대한 초과 및 미만 비교 연상을 수행
    // 경게값을 포함할 경우 Equal 키워드를 추가

    List<ProductEntity> findByPriceIsGreaterThan(Long price); // 지정한 가격을 초과하는 데이터 조회
    ...
    List<ProductEntity> findByPriceIsLessThan(Long price); // 지정하 가격 미만 데이터 조회
    ...
    List<ProductEntity> findByPriceIsBetween(Long lowPrice, Long highPrice); // 지정한 낮은 데이터, 높은 데이터 까지의 데이터를 조회
    ...
    List<ProductEntity> findByNameContaining(String name); // 지정한 문자열이 양 끝에 포함되어 있는 이름 조회
...
//    List<ProductEntity> findByNameOrderByNumberAsc(Long number); // 지정한 이름으로 데이터를 조회 후 번호 기준으로 오름차순 정렬
//    List<ProductEntity> findByNameOrderByNumberDesc(Long number); // 지정한 이름으로 데이터를 조회 후 번호를 기준으로 내림차순 정렬
//    List<ProductEntity> findByNameOrderByPriceAscStockDesc(Long number); // 지정한 이름으로 데이터를 조회 후 가격을 기준으로 오름차순 정렬한 뒤, 재고수량을 기준으로 내림차순 정렬

    // 정렬 사용 시 Sort객체를 사용하여 정렬하는 것이 가능함
    // 쿼리 메소드 생성 후 매개변수에 Sort 객체를 추가하여 정렬
    // List<ProductEntity> findByName(Long number);와 같이 쿼리 메소드 생성 후 실행 시에 Sort 객체를 추가하여 실행
    ...
    // @Query : 복잡한 쿼리문은 쿼리 메소드로 만들기 힘들기 때문에 @Query 라는 어노테이션을 제공함
    // JPQL(JPA SQL) 문법을 사용해 SQL 쿼리를 생성하여 복잡한 쿼리문을 실행할 수 있도록 도와줌
    // 검색 조건에 "?순번" 형태로 데이터를 사용함
    // 사용법
    // @Query("select entity 객체명 from entity 클래스명 WHERE 검색 조건1, 검색조건2, ...")
    ...
    // 전체 컬럼을 출력 시 클래스의 별명만 입력
    // WHERE 절에는 클래스 별명.컬럼명 형태로 사용함
    @Query("SELECT p FROM ProductEntity AS p WHERE p.name = '상품2'")
    ProductEntity querySelectName();


    // 메소드의 매개변수를 쿼리문에 적용하고자 할 경우 ?순번 형식으로 입력하여 데이터를 매칭함
    ...
    // :매개변수 방식은 메소드의 매개변수 앞에@Param("컬렴명") 을 반드시 사용해야 함
    // :매개변수 방식은 메소드의 매개변수 순서와 상관이 없음
    @Query("SELECT p FROM ProductEntity AS p WHERE p.name = :name AND p.price = :price")
    ProductEntity querySelectNameAndPrice(@Param("name")String name, @Param("price")int price);
}

// 문제 1) employees 데이터베이스를 자신의 mysql server에 import하고 해당 데이터 베이스의
// 본인계정에 사용권한을 설정한 후 entity 및 repository 작성하고 데이터를 검색하는 프로그램 작성
// 1. root 계정으로 employess DB를 생성하고 DB내용을 import 하여 복구
// 2. 일반 계정에 employees DB에 대한 사용 권한 설정
