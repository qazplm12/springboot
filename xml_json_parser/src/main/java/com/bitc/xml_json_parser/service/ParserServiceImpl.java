...
//        List<PharmacyFullDataItemDTO> itemList = items.getItemList();
    ...
    public List<DailyBoxOfficeDTO> getDailyBoxOfficeList(String strUrl) throws Exception {
        ...
        try {
            ...
            // BufferedReader를 사용하여 HttpURLConnection 을 통해서가져온 데이터를 읽어옴
            // JAXB 라이브러리는 클래스 내부에 네트워크 데이터 처리 부분이 이미 존재함
            // 현재는 JSON 데이터를 가져옴
            // 네트워크 통신 시 사용하는 JSON 데이터는 Javascript의 Object 타입을 흉내낸 문자열
            reader = new BufferedReader(new InputStreamReader(urlConn.getInputStream()));

            // 자바에서 문자열을 표현하기 위한 클래스인 String 클래스는 문자열을 저장하고 하나의 문자열을 저장하고
            // 해당 문자열에 다른 문자열을 연결할 경우 실제로는 기존 문자열에 다른 문자열이 추가되는 것이 아니라
            // 기존 문자열과 추가된 문자열이 합쳐진 새 문자열을 생성

            // 네트워크 통신으로 들어오는 문자열 데이터를 처리하기에는 기존 String 클래스는 비효율적이므로
            // String Builder 클래스를 사용함
            ...
            Gson gson = new Gson();
            // json 문자열을 자바의 오브젝트 타입으로 변경
            // fromJson(파싱할 문자열, 파싱할 자바 데이터 타입)
