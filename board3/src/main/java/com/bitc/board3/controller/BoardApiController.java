...
// @Controller 와 @ResponseBody 를 함께 사용해도 동일한 효과를 가짐
// ajax 통신 시 rest api 방식으로 데이터를 전송하는 컨트롤러를 만들어서 사용하면 편리함
// 리액트 사용 시 프론트엔드 부분과 백엔드 부분이 완전히 분리되어 있기 때문에 데이터를 주고받기 위해서 ajax 통신이 필요
...
public class BoardApiController {
    ...
    // RESTful API : REST 방식과 동일한 형태로 HTTP 프로토콜을 사용하지만, 클라이언트로 전달하는 데이터가
    // View를 제외한 데이터만 전달하는 방식
    // 예전에는 클라이언트가 컴퓨터밖에 없었기 때문에 웹 브라우저가 클라이언트의 기능을 담당 했음
